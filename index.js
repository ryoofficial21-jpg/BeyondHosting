...
client.on(Events.MessageCreate, async (message) => {
    ...
    if (message.content !== "!monitor") return;
    ...
    monitorInterval = setInterval(async () => {
        try {
            ...
            const MEMORY_LIMIT_MB = 8192; // 8GB
            const CPU_LIMIT = 8; // 8 vCPU
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    {
                        name: "🧠 Memory Usage",
                        value: `${ramUsed} MiB / ${MEMORY_LIMIT_MB} MiB`,
                        inline: false
                    },
                    {
                        name: "⚡ CPU Usage",
                        value: `${cpuUsage}% / ${CPU_LIMIT} vCPU`,
                        inline: false
                    },
                    {
                        name: "⏱️ Uptime",
...
if (
    message.channel.id === config.interview.channelId &&
    content.includes("Name:") &&
    content.includes("Steam Link:")
) {
...
client.on(Events.InteractionCreate, async (interaction) => {
    ...
    const allowed =
        moderator.roles.cache.has(config.roles.moderator) ||
        moderator.roles.cache.has(config.roles.admin) ||
        moderator.roles.cache.has(config.roles.headAdmin);
...
await target.roles.remove(
    config.serverRoles.unverified
).catch(() => {});

// Kunin yung Name field sa embed
const rpName = fields[1].value;

// Palitan nickname
await target.setNickname(rpName).catch(() => {});
