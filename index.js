...
async function sendLog(channelId, embed, guild) {
    try {
...
return interaction.update({
    ...
    .addFields(
        ...
        { name: "Name", value: fields[2]?.value || "N/A" },
        { name: "Steam Link", value: fields[3]?.value || "N/A" },
        { name: "Time", value: new Date().toISOString() }
    );

await sendLog(config.logs.whitelist, logEmbed, interaction.guild);
...
if (action === "admin") {
    ...
    // 🔥 DITO MO ILAGAY YUNG LOG (AFTER SUCCESS)
const logEmbed = new EmbedBuilder()
    .setTitle(roleType === "role" ? "Role Approved" : "Unrole Approved")
    .setColor("Green")
    ...
    .addFields(
        ...
        { name: " Type", value: roleType, inline: true },
        { name: " Time", value: `<t:${Math.floor(Date.now()/1000)}:F>`, inline: false }
    )
    .setTimestamp();

    await sendLog(
        roleType === "role" ? config.logs.role : config.logs.unrole,
        logEmbed,
        interaction.guild
    );
}
